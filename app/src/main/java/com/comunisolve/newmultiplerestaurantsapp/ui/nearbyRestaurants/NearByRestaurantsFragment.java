package com.comunisolve.newmultiplerestaurantsapp.ui.nearbyRestaurants;

import androidx.lifecycle.ViewModelProvider;

import android.os.Bundle;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.comunisolve.newmultiplerestaurantsapp.R;

public class NearByRestaurantsFragment extends Fragment {

    private NearByRestaurantsViewModel mViewModel;

    public static NearByRestaurantsFragment newInstance() {
        return new NearByRestaurantsFragment();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.near_by_restaurants_fragment, container, false);
    }

    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mViewModel = new ViewModelProvider(this).get(NearByRestaurantsViewModel.class);
        // TODO: Use the ViewModel
    }

}